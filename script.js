...
const Axiom = {
  ...
  bind(){
    ...
    mainCol.addEventListener("click",e=>{
      ...
      if(o){ e.preventDefault(); this.openSite(o.dataset.open,o.dataset.title||o.dataset.open,o.dataset.embed==="1") }
    });
    ...
    saveSettings.addEventListener("click",()=>Settings.save());
    clearHistoryBtn.addEventListener("click",()=>{History.clear(); this.renderRecent(); this.toastMsg("History cleared")});
    settingsModal.addEventListener("click",e=>{if(e.target.id==="settingsModal") Settings.close()});
    recentList.addEventListener("click",e=>{const x=e.target.closest(".x"); if(x){e.stopPropagation(); History.remove(parseInt(x.dataset.i)); this.renderRecent(); return} const item=e.target.closest(".recent-item"); if(item) this.search(item.dataset.q)});
  ...
  async search(q){
    ...
    History.add(q);
    this.renderRecent();
    home.classList.add("hidden"); results.classList.remove("hidden"); tbInput.value=q;
    mainCol.innerHTML=""; sideCol.innerHTML=""; sideCol.classList.add("hidden"); layout.classList.remove("has-panel");
    ...
    let rt=null;
    const schedule=()=>{ clearTimeout(rt); rt=setTimeout(()=>{ if(this.state.results!==data) return; if(data.primaryDone) this.loading(false); this.render(); },60) };
    const finish=()=>{ data.primaryDone=true; schedule() };
  ...
  relevanceScore(q,text,bonus=0){
    ...
    for(const t of terms) if(text.includes(t)) s+=4;
  ...
  async buildSiteDirectory(q){
    ...
    return [...scored,...extra].sort((a,b)=>b.score-a.score).slice(0,120);
  },
  canEmbed(domain){
  ...
  openSite(url,title,embed){
    ...
    this.el.sideCol.innerHTML = `<div class="panel"><h3>${this.esc(title||url)}</h3><div class="meta">${this.esc(url)}</div><div class="launch"><button class="btn primary" data-open="${this.esc(url)}" data-title="${this.esc(title||url)}" data-embed="0">Open</button><button class="btn" id="closeViewer">Close</button></div><div class="viewer"><div class="viewer-head"><div class="u">${this.esc(url)}</div></div>${frame}</div></div>`;
    closeViewer.addEventListener("click",()=>{ this.el.sideCol.classList.add("hidden"); this.el.layout.classList.remove("has-panel"); this.el.sideCol.innerHTML="" });
  ...
  renderSummary(d){
    ...
    html += `<div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap"><button class="btn" data-search="${this.esc(this.state.q+" site:google.com")}">Google</button><button class="btn" data-search="${this.esc(this.state.q+" site:wikipedia.org")}">Wikipedia</button><button class="btn" data-search="${this.esc(this.state.q+" site:archive.org")}">Archive</button></div>`;
    html += `</div></div>`;
    this.el.mainCol.innerHTML=html;
  },
  renderWeb(d){
    let html="";
    if(d.webError) html+=this.emptyState("Web results unavailable",d.webError);
    else if(!d.web.length) html+=this.emptyState("No web results","Try different keywords.");
    else html+=d.web.map(r=>this.resultCard(r,"web")).join("");
    if(d.archive.length){ html+=`<div class="src-badge b-archive">Internet Archive</div>` + d.archive.slice(0,6).map(a=>this.resultCard(a,"archive")).join(""); }
    if(d.encyclopedia.length){ html+=`<div class="src-badge b-encyclopedia">Encyclopedia.com</div>` + d.encyclopedia.slice(0,6).map(e=>this.resultCard(e,"encyclopedia")).join(""); }
    this.el.mainCol.innerHTML=html;
    this.applyNewtab(this.el.mainCol);
  },
  renderSites(d){
    if(!d.sites.length){ this.el.mainCol.innerHTML=this.emptyState("No site matches","Try a different query."); return; }
    this.el.mainCol.innerHTML=`<div class="site-list">`+d.sites.map(s=>`<div class="site-row"><div><div class="name">${this.esc(s.name)}</div><div class="domain">${this.esc(s.domain||this.hostOf(s.url))}</div></div><button class="open" data-open="${this.esc(s.url)}" data-title="${this.esc(s.name)}" data-embed="${s.embed?1:0}">Open</button></div>`).join("")+`</div>`;
  },
  renderAcademic(d){ this.el.mainCol.innerHTML=d.academic.length?d.academic.map(a=>this.resultCard(a,"academic")).join(""):this.emptyState("No academic papers","No scholarly works found."); this.applyNewtab(this.el.mainCol); },
  renderCode(d){ this.el.mainCol.innerHTML=d.code.length?d.code.map(c=>this.resultCard(c,"code")).join(""):this.emptyState("No repositories","No GitHub repositories matched."); this.applyNewtab(this.el.mainCol); },
  renderImages(d){
    if(!d.images.length){ this.el.mainCol.innerHTML=this.emptyState("No images","No images found."); return; }
  ...
  renderChips(){ const c=["google","youtube","wikipedia","reddit","github","news","maps","archive.org","encyclopedia.com","crossref","openlibrary","stack overflow","bing","duckduckgo","amazon","facebook"]; homeChips.innerHTML=c.map(x=>`<button class="chip" type="button">${this.esc(x)}</button>`).join(""); homeChips.addEventListener("click",e=>{const b=e.target.closest(".chip"); if(b) this.search(b.textContent)}); },
  ...
  applyTheme(){ document.documentElement.setAttribute("data-theme",Settings.obj.theme||"dark") },
